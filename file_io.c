               ...
               int       *membership)   /* [numObjs] */
{
    ...
    sprintf(outFileName, "%s.centros_clusters", filename);
    printf("Coordenadas para K=%d centrode de clusters al archivo \"%s\"\n",
    ...
    sprintf(outFileName, "%s.ordenado", filename);
    printf("Escribiendo nuevos datos de N=%d objetos al archivo \"%s\"\n",
           numObjs, outFileName);
    fptr = fopen(outFileName, "w");
    for (i=0; i<numObjs; i++)
        fprintf(fptr, "%d %d\n", i, membership[i]);
